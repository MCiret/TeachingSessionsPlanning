...
async def test_create_availability(db_tests: AsyncSession) -> None:
    ...
    assert got_avail.end_date == date(2022, 3, 31)
...
async def test_update_availability(db_tests: AsyncSession) -> None:
    ...
    avail_in = AvailabilityUpdate(end_date=date(2022, 3, 15))
    await crud.availability.update(db_tests, db_obj=avail, obj_in=avail_in)
    got_avail = await crud.availability.get(db_tests, id=avail.id)
    assert got_avail.speaker_id == speaker.id
    assert got_avail.start_date == date(2022, 1, 1)
    assert got_avail.end_date == date(2022, 3, 15)
    await crud.availability.remove(db_tests, id=avail.id)


async def test_is_start_before_end_date() -> None:
    assert await crud.availability.is_start_before_end_date(start_date=date(2021, 1, 8),
                                                            end_date=date(2022, 1, 12))
    assert not await crud.availability.is_start_before_end_date(start_date=date(2022, 1, 8),
...
async def test_is_a_good_weekday_int() -> None:
    ...
    assert await crud.availability.is_a_good_weekday_int(start_date=startdate, end_date=date(2022, 2, 9),
                                                         weekday_int=4)
    assert await crud.availability.is_a_good_weekday_int(start_date=startdate, end_date=date(2022, 2, 5),
                                                         weekday_int=5)
    assert not await crud.availability.is_a_good_weekday_int(start_date=startdate, end_date=date(2022, 2, 5),
                                                             weekday_int=6)
    assert await crud.availability.is_a_good_weekday_int(start_date=startdate, end_date=date(2022, 2, 2),
                                                         weekday_int=2)
    assert not await crud.availability.is_a_good_weekday_int(start_date=startdate, end_date=date(2022, 2, 2),
                                                             weekday_int=0)
...
class TestCrudAvailabilityOne:
    ...
    async def test_get_by_speaker(self, db_avails, db_tests: AsyncSession) -> None:
        ...
        avails_spk1 = await crud.availability.get_by_speaker(db_tests, speaker_id=spk1_id)
        ...
        avails_spk2 = await crud.availability.get_by_speaker(db_tests, speaker_id=spk2_id)
    ...
    async def test_get_by_date_speaker(self, db_avails, db_tests: AsyncSession) -> None:
        spk1_id = db_avails["speaker1"].id
        # thursday
        avail_thur_14_dec21 = await crud.availability\
                                        .get_by_date_speaker(db_tests, speaker_id=spk1_id, date=date(2021, 12, 14))
        ...
        assert not await crud.availability\
                             .get_by_date_speaker(db_tests, speaker_id=spk1_id, date=date(2022, 4, 6))
        # tuesday
        avail_tue_7_april = await crud.availability\
                                      .get_by_date_speaker(db_tests, speaker_id=spk1_id, date=date(2022, 4, 7))
        ...
        avail_thur_8_march = await crud.availability\
                                       .get_by_date_speaker(db_tests, speaker_id=spk1_id, date=date(2022, 3, 8))
        assert len(avail_thur_8_march) == 2
        for av in avail_thur_8_march:
            assert av.speaker_id == spk1_id

    async def test_get_by_date_time_speaker(self, db_avails, db_tests: AsyncSession) -> None:
        spk1_id = db_avails["speaker1"].id
        # thursday
        avail_thur_14_dec21 = await crud.availability\
                                        .get_by_date_time_speaker(db_tests, speaker_id=spk1_id,
                                                                  date=date(2021, 12, 14), time=time(9, 30))
        assert avail_thur_14_dec21.id == db_avails["avail3_spk1"].id
        # thursday
        assert not await crud.availability\
                             .get_by_date_time_speaker(db_tests, speaker_id=spk1_id,
                                                       date=date(2021, 12, 14), time=time(9))
        # thursday
        avail_thur_25_jan_9h30 = await crud.availability\
                                           .get_by_date_time_speaker(db_tests, speaker_id=spk1_id,
                                                                     date=date(2022, 1, 25),  time=time(9, 30))
        assert avail_thur_25_jan_9h30.id == db_avails["avail3_spk1"].id
        # thursday
        avail_thur_25_jan_9h = await crud.availability\
                                         .get_by_date_time_speaker(db_tests, speaker_id=spk1_id,
                                                                   date=date(2022, 1, 25), time=time(9))
        assert avail_thur_25_jan_9h.id == db_avails["avail1_spk1"].id
        # friday
        assert not await crud.availability\
                             .get_by_date_time_speaker(db_tests, speaker_id=spk1_id,
                                                       date=date(2022, 1, 28), time=time(9))
    ...
    async def test_get_by_weekday_time_speaker(self, db_avails, db_tests: AsyncSession) -> None:
        ...
        assert (await crud.availability
                          .get_by_weekday_time_speaker(db_tests, speaker_id=spk1_id,
                                                       week_day=1,
                                                       time=time(9, 30)))[0].id == db_avails["avail3_spk1"].id
        # thursday
        assert (await crud.availability
                          .get_by_weekday_time_speaker(db_tests, speaker_id=spk1_id,
                                                       week_day=1, time=time(9)))[0].id == db_avails["avail1_spk1"].id
        # tuesday
        assert (await crud.availability
                          .get_by_weekday_time_speaker(db_tests, speaker_id=spk1_id,
                                                       week_day=3, time=time(10)))[0].id == db_avails["avail2_spk1"].id
        # tuesday
        assert not await crud.availability\
                             .get_by_weekday_time_speaker(db_tests, speaker_id=spk1_id, week_day=3, time=time(11))
        # thursday
        assert not await crud.availability\
                             .get_by_weekday_time_speaker(db_tests, speaker_id=spk1_id, week_day=1, time=time(11))
    ...
    async def test_has_too_close_previous_false(self, db_avails, db_tests: AsyncSession) -> None:
        spk1_id = db_avails["speaker1"].id
        # thursday
        avail_in_1 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=1, time=time(10))
        # thursday
        avail_in_2 = AvailabilityCreate(start_date=date(2022, 1, 25), end_date=date(2022, 6, 25),
                                        week_day=1, time=time(8, 30))
        # friday
        avail_in_3 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=4, time=time(10))
        # tuesday
        avail_in_4 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=3, time=time(10, 30))
        # thursday
        avail_in_5 = AvailabilityCreate(start_date=date(2022, 6, 25), end_date=date(2022, 8, 25),
                                        week_day=1, time=time(8, 30))
        ...
        spk2_id = db_avails["speaker2"].id
        # thursday
        avail_in_6 = AvailabilityCreate(start_date=date(2022, 1, 25), end_date=date(2022, 6, 25),
                                        week_day=1, time=time(11, 20))
        # wednesday
        avail_in_7 = AvailabilityCreate(start_date=date(2022, 3, 25), end_date=date(2022, 6, 25),
                                        week_day=2, time=time(14))
    ...
    async def test_has_too_close_previous_true(self, db_avails, db_tests: AsyncSession) -> None:
        ...
        # thursday
        avail_in_1 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=1, time=time(9, 10))
        # thursday
        avail_in_2 = AvailabilityCreate(start_date=date(2022, 1, 25), end_date=date(2022, 6, 25),
                                        week_day=1, time=time(9, 40))
        # tuesday
        avail_in_3 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=3, time=time(10, 25))
        ...
        spk2_id = db_avails["speaker2"].id
        # thursday
        avail_in_4 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 4, 25),
                                        week_day=1, time=time(11, 15))
    ...
    async def test_has_too_close_next_false(self, db_avails, db_tests: AsyncSession) -> None:
        ...
        # friday
        avail_in_3 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=4, time=time(14))
        # tuesday
        avail_in_4 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=3, time=time(9, 30))
        # thursday
        avail_in_5 = AvailabilityCreate(start_date=date(2022, 7, 25), end_date=date(2022, 10, 25),
                                        week_day=1, time=time(10))
        ...
        spk2_id = db_avails["speaker2"].id
        # thursday
        avail_in_6 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 4, 25),
                                        week_day=1, time=time(10, 40))
    ...
    async def test_has_too_close_next_true(self, db_avails, db_tests: AsyncSession) -> None:
        spk1_id = db_avails["speaker1"].id
        # thursday
        avail_in_1 = AvailabilityCreate(start_date=date(2021, 11, 25), end_date=date(2022, 3, 25),
                                        week_day=1, time=time(8, 45))
        # thursday
        avail_in_2 = AvailabilityCreate(start_date=date(2022, 1, 25), end_date=date(2022, 6, 25),
                                        week_day=1, time=time(9, 20))
        # thursday
        avail_in_3 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 3, 25),
                                        week_day=1, time=time(9, 10))
        # tuesday
        avail_in_4 = AvailabilityCreate(start_date=date(2021, 12, 25), end_date=date(2022, 7, 25),
                                        week_day=3, time=time(9, 35))
        # tuesday
        avail_in_5 = AvailabilityCreate(start_date=date(2021, 10, 25), end_date=date(2022, 8, 25),
                                        week_day=3, time=time(9, 50))
        ...
        spk2_id = db_avails["speaker2"].id
        # thursday
        avail_in_6 = AvailabilityCreate(start_date=date(2022, 3, 25), end_date=date(2022, 7, 25),
                                        week_day=1, time=time(10, 45))
        # thursday
        avail_in_7 = AvailabilityCreate(start_date=date(2021, 10, 25), end_date=date(2022, 8, 25),
                                        week_day=1, time=time(10, 50))
...
class TestCrudAvailabilityTwo:
    ...
    async def db_avails(self, db_tests: AsyncSession) -> None:
        ...
        speaker1 = await ut.create_random_speaker(db_tests, slot_time=30)
        avail1_spk1 = await crud.availability.create(db_tests,
                                                     obj_in=AvailabilityCreate(start_date=date(2022, 2, 1),
                                                                               end_date=date(2022, 5, 31),
                                                                               week_day=1,
                                                                               time=time(9)),
                                                     speaker_id=speaker1.id)
    ...
    async def test_is_same_period_weekday_exists(self, db_avails, db_tests: AsyncSession) -> None:
        spk1_id = db_avails["speaker1"].id
        avail_in_1 = AvailabilityCreate(start_date=date(2021, 12, 1), end_date=date(2022, 1, 31),
                                        week_day=1, time=time(9))
        avail_in_2 = AvailabilityCreate(start_date=date(2022, 1, 15), end_date=date(2022, 2, 28),
                                        week_day=1, time=time(9))
        avail_in_3 = AvailabilityCreate(start_date=date(2022, 2, 20), end_date=date(2022, 4, 30),
                                        week_day=1, time=time(9))
        avail_in_4 = AvailabilityCreate(start_date=date(2022, 3, 15), end_date=date(2022, 5, 30),
                                        week_day=1, time=time(9))
        avail_in_5 = AvailabilityCreate(start_date=date(2022, 6, 2), end_date=date(2022, 8, 31),
                                        week_day=1, time=time(9))
        avail_in_6 = AvailabilityCreate(start_date=date(2021, 12, 1), end_date=date(2022, 4, 30),
                                        week_day=4, time=time(14))
        avail_in_7 = AvailabilityCreate(start_date=date(2022, 2, 1), end_date=date(2022, 8, 31),
                                        week_day=4, time=time(14))
        avail_in_8 = AvailabilityCreate(start_date=date(2021, 12, 1), end_date=date(2022, 8, 31),
                                        week_day=3, time=time(14))
        spk2_id = db_avails["speaker2"].id

        assert not await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk1_id,
                                                                         obj_in=avail_in_1)
        assert await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk1_id,
                                                                     obj_in=avail_in_2)
        assert await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk1_id,
                                                                     obj_in=avail_in_3)
        assert await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk1_id,
        ...
        spk2_id = db_avails["speaker2"].id
        assert not await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk2_id,
                                                                         obj_in=avail_in_1)
        assert await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk2_id,
                                                                     obj_in=avail_in_2)
        assert await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk2_id,
                                                                     obj_in=avail_in_3)
        assert await crud.availability.is_same_period_weekday_exists(db_tests, speaker_id=spk2_id,
    ...
    async def test_is_same_period_weekday_time_exists(self, db_avails, db_tests: AsyncSession) -> None:
        spk1_id = db_avails["speaker1"].id
        avail_in_1 = AvailabilityCreate(start_date=date(2021, 12, 1), end_date=date(2022, 1, 31),
                                        week_day=1, time=time(9))
        avail_in_2 = AvailabilityCreate(start_date=date(2022, 1, 15), end_date=date(2022, 2, 28),
        ...
        avail_in_6 = AvailabilityCreate(start_date=date(2021, 12, 1), end_date=date(2022, 4, 30),
                                        week_day=4, time=time(14))
        avail_in_7 = AvailabilityCreate(start_date=date(2021, 12, 1), end_date=date(2022, 8, 31),
                                        week_day=4, time=time(14))
        assert not await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk1_id,
                                                                              obj_in=avail_in_1)
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk1_id,
                                                                          obj_in=avail_in_2)
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk1_id,
                                                                          obj_in=avail_in_3)
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk1_id,
        ...
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk1_id,
                                                                          obj_in=avail_in_7)
        spk2_id = db_avails["speaker2"].id
        assert not await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk2_id,
                                                                              obj_in=avail_in_1)
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk2_id,
                                                                          obj_in=avail_in_2)
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk2_id,
                                                                          obj_in=avail_in_3)
        assert await crud.availability.is_same_period_weekday_time_exists(db_tests, speaker_id=spk2_id,
